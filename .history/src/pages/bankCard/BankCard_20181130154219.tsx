...
export class BankCard extends React.Component<BankCardProps, BankCardState> {
    ...
    getBankCard = () => {
        UserService.Instance.addPayment().then( (res:any) => {
            this.setState({
          ...
          const row = (rowData: model.exChangeRecordItem, sectionID: number, rowID: number) => {
            ...
            return (
                <div style={{height:".6rem",marginTop:".01rem"}}>
                    <div style={{float:"left",padding:".1rem"}}>{rowData.coid_name}</div>
                    <div style={{float:"right",padding:".1rem"}}>{rowData.orderid}</div>
                </div>
