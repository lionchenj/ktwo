...
export class Wallet extends React.Component<WalletProps, WalletState> {
    ...
    onSubmit = () => {
        ...
        UserService.Instance.assets_move(this.state.selectedCoinId, this.state.changeCoin, this.WalletNumber, this.gesturePasswords, this.state.bankName, this.state.bankId, this.state.service).then(() => {
            UIUtil.hideLoading();
            Modal.alert('提示', '提取成功', [{
                text: 'ok', onPress: () => {
                    this.props.history.goBack()
                    ...
                    <div style={{ height: bodyHeight, backgroundColor: '#f5f5f5' }}>
                        <WingBlank>
                            ...
                            <List className="change-list">
                                <List.Item extra={this.state.bankName}>银行名称</List.Item>
                                <List.Item extra={this.state.bankId}>银行卡号</List.Item>
                            </List>
                            <WhiteSpace size="lg" />
                            <WhiteSpace size="lg" />
                            {/* <div className='wallet-list-title'>矿工费</div>
