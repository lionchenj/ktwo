...
export class Change extends React.Component<ChangeProps, ChangeState> {
    ...
    onSubmit = (event: React.MouseEvent) => {
        ...
        const gesturePasswordInfo = "请输入交易密码"
        ...
        if (!this.state.gesturePassword) {
            UIUtil.showInfo(gesturePasswordInfo)
            return
        }
        UIUtil.showLoading("转换中")
        UserService.Instance.give(this.changeNumber, trimPhone, this.state.gesturePassword).then( () => {
            UIUtil.hideLoading()
